  ...
  return (
    <div className="fixed z-50 inset-0 overflow-y-auto">
      <div className="flex flex-col items-center justify-center  min-h-[50vh] rounded-lg pt-4 px-4 pb-20 m-auto max-w-md md:max-w-xl text-center sm:block sm:p-0">
        ...
        <div
          className={`inline-block align-bottom w-full ${(result.users?.length > 0 || result.posts?.length > 0) ? `bg-bgColor` : `bg-primary`} rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle `}
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          <div className="px-4 sm:px-6 flex flex-col gap-3 2xl:gap-6">
            {/* USERS */}
            <div>
              {result.users?.length > 0 && <h3 className=" text-xl text-blue mb-4">Peoples</h3>}
              {result.users?.map((user) => (
                ...
                >
                  <div onClick={handleSearchBoxClose} className="pt-4">
                    <Link to={"/profile/" + user?._id} className="flex gap-2 ml-3">
                      <img
            ...
            <h3 className=" text-xl text-blue mb-4">Posts</h3>
              {loading ? (
                ...
                <div className="flex w-full h-full items-center justify-center">
                  <p className="text-lg text-ascent-2 mb-8">No Post Available</p>
                </div>
              )}
            </div>
          </div>
